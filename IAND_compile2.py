...
def compile_data(filename):
    ...
    open("IAND_v1_preprocessed.txt", "w").writelines([l for l in open(filename[:-4]+'_jaccard_alliances_conflicts_freedom_regions_trade.txt').readlines()])    
    stop=time.time()
    convert_to_csv("IAND_v1_preprocessed.txt")
    print "done compiling IAND"
    print "this compilation took: ", stop-start
    print "final file name: IAND_v1_preprocessed.txt"
    return 0
compile_data("temp_build_IAND_v1.txt")
